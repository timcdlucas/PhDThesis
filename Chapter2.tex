...
\begin{alltt}
\hlcom{# Compilation options}
\hlcom{# Run simulations? This will take many hours}
\hlstd{runAllSims} \hlkwb{<-} \hlnum{FALSE}


\hlstd{opts_chunk}\hlopt{$}\hlkwd{set}\hlstd{(}\hlkwc{cache.path} \hlstd{=} \hlstr{'.Ch2Cache/'}\hlstd{)}

\hlkwd{source}\hlstd{(}\hlstr{'misc/theme_tcdl.R'}\hlstd{)}
\hlkwd{source}\hlstd{(}\hlstr{'misc/KnitrOptions.R'}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
