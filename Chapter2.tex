...
After \ensuremath{4\times 10^{4}} events, 10 individuals infected with disease 2 are added to one colony. 
After another \ensuremath{4.6\times 10^{5}} events the invasion of disease 2 is considered successful if any individuals with disease 2 still remain.
...
{\centering \subfloat[Dispersal\label{f:invasionPropPlots1}]{\includegraphics[width=0.45\textwidth]{figure/invasionPropPlots-1} }
\subfloat[Network topology\label{f:invasionPropPlots2}]{\includegraphics[width=0.45\textwidth]{figure/invasionPropPlots-2} }
...
This was true at all transmission levels ($\chi^2$ test. $\beta = 2$: $\chi^2 = \ensuremath{NaN}$, $\text{df} = 2$, $p = \ensuremath{NaN}$. $\beta = 5$: $\chi^2 = \ensuremath{NaN}$, $\text{df} = 2$, $p = \ensuremath{NaN}$. $\beta = 10$: $\chi^2 = \ensuremath{NaN}$, $\text{df} = 2$, $p = \ensuremath{NaN}$).
...
The proportion of invasions was not different between highly connected and largely unconnected metapopulations (See Figure~\ref{f:invasionPropPlots1}). 
This was true at all transmission levels ($\chi^2$ test. $\beta = 2$: $\chi^2 = \ensuremath{NaN}$, $\text{df} = 1$, $p = \ensuremath{NaN}$. $\beta = 5$: $\chi^2 = \ensuremath{NaN}$, $\text{df} = 1$, $p = \ensuremath{NaN}$. $\beta = 10$: $\chi^2 = \ensuremath{NaN}$, $\text{df} = 1$, $p = \ensuremath{NaN}$).
...
However, the results are the same as above if these simulations are removed from the analysis ($\chi^2$ test. $\beta = 2$: $\chi^2 = \ensuremath{NaN}$, $\text{df} = 1$, $p = \ensuremath{NaN}$. $\beta = 5$: $\chi^2 = \ensuremath{NaN}$, $\text{df} = 1$, $p = \ensuremath{NaN}$. $\beta = 10$: $\chi^2 = \ensuremath{NaN}$, $\text{df} = 1$, $p = \ensuremath{NaN}$).
...
This is true for all three dispersal values ($\chi^2$ test. $\lambda = 0.001$: $\chi^2 = \ensuremath{NaN}$, $\text{df} = 2$, $p = \ensuremath{NaN}$. $\lambda = 0.01$: $\chi^2 = \ensuremath{NaN}$, $\text{df} = 2$, $p = \ensuremath{NaN}$. $\lambda = 0.1$: $\chi^2 = \ensuremath{NaN}$, $\text{df} = 2$, $p = \ensuremath{NaN}$) and both network structures ($\chi^2$ test. Fully connected: $\chi^2 = \ensuremath{NaN}$, $\text{df} = 2$, $p = \ensuremath{NaN}$. Minimally connected: $\chi^2 =  \ensuremath{NaN}$, $\text{df} = 2$, $p = \ensuremath{NaN}$).
