...
Only the model with study effort, gene flow and mass is well supported with the second model having an $\Delta\text{AICc}$ of 34 (Table \ref{t:models}).
While less strongly supported than the number of subspecies, gene flow is likely in the best model ($Pr = $ 0.89) compared to the benchmark random variable which has $Pr = $ 0.18 (Figure~\ref{fig:fstITPlots}B and Table~\ref{t:variables}).
...
Possibly due to the smaller sample size, or a weaker relationship, this coefficient is much more varied than the number of subspecies coeficient with 22\% of models estimating a positive relationship.
...
Of the three independant variables in the best model, study effort has the largest effect ($\beta = $ 2.49, variance = 0.08).
The effect size of gene flow ($\beta = $ \ensuremath{-0.67}, variance = \ensuremath{5.48\times 10^{-3}}) is approximately twice the size of that of body mass ($\beta = $ \ensuremath{-0.35}, variance = 0.04)
