...
To establish the importance of variables I calculated the probability that each variable would be in the best model if the data were recollected.
This value is called the variables weight.
...
The whole analysis was run 10 times, resampling the random variable each time.
...
From these $\Delta\text{AICc}$ calculated Akaiki weights.
...
The number of subspecies is very likely in the best model ($Pr > $ 4.99) as is the interaction between number of species and study effort ($Pr = $ 4.86) compared to the benchmark random variable which has $Pr = $ 1.
When models with the interaction term are removed, on average (mean weighted by Akaiki weights) there is a positive relationship between the number of subspecies and viral richness ($\beta = $ 0.54, variance = \ensuremath{1.37\times 10^{-3}}).
Models with an interaction between number of subspecies and study effort have a positive interaction term ($\beta = $ 0.5, variance = \ensuremath{5.03\times 10^{-5}}) and a negative linear term ($\beta = $ 0.31, variance = \ensuremath{2.12\times 10^{-4}}) which in total is a positive relationship for most of the range of the study effort data.
...
As seen in Figure~\ref{fig:fstITPlots}a, study effort is very likely in the best model ($\beta = $ 0.99, $Pr > $ 4.99) body mass and range size are also probably in the best model ($\beta = $ 0.48, $Pr = $ 3.98 and $\beta = $ 0.34, $Pr = $ 3.28).
...
\hspace{3mm}Total & 5.00 & 0.32 &&\\
\hspace{3mm}Models without interaction term &&  0.54 &&\\
\hspace{3mm}Models with interaction term &&  0.31 &&\\
Number of subspecies:log(Scholar) &  4.86 &  0.50 && \\[2mm]  
Gene flow & & &  4.45 &  \ensuremath{-0.67}\\[2mm]  
log(Scholar) &  5.00 &  0.99 & 
   4.94 &  2.49\\
log(Mass) &  3.98 &  0.48 & 
   4.9 &  \ensuremath{-0.35}\\
log(Range size) &  3.28 &  0.34& 
   0.31 &  1.57\\
Random &  1.00 &  0.05& 
   0.92 &  0.23\\
\bottomrule
...
$\bar{\text{AICc}}$ is the mean AICc score across 10 resamplings of the null random variable. 
...
In the gene flow analysis, the top two models are considerably better than the other models; there is a 100\% chance that one of them is the best model.
...
While less strongly supported than the number of subspecies, gene flow is likely in the best model ($Pr = $ 4.45) compared to the benchmark random variable which has $Pr = $ 0.92 as shown in Figure~\ref{fig:fstITPlots}b.
...
As in the number of subspecies analysis, study effort is very likely in the best model ($Pr = $ 4.94) as is body mass ($Pr = $ 4.9).
In contrast to the number of subspecies analysis, range size is almost certainly not in the best model with $Pr = $ 0.31 being much less than the random variable.
