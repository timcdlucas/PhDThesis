...
Summing the Akaiki weights of all models with a given variable in gives a probability that the variable would be in the best model if the data were recollected \cite{whittingham2006we}.
The number of subspecies is very likely in the best model ($P(\text{in best model}) = $ 1, 1, 0.8, 0.66, 0.2, 0.97).
When models with an interaction term are removed, on average (mean weighted by Akaiki weights) there is a positive relationship between the number of subspecies and viral richness ($\beta = $ 0.19, variance = \ensuremath{1.24\times 10^{-4}}).
