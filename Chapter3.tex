...
Summing the Akaiki weights of all models that contain a given variable gives a probability that the variable would be in the best model (Figure~\ref{fig:ITPlots}) if the data were recollected \cite{whittingham2006we}.
The number of subspecies is very likely in the best model ($P(\text{in best model}) > $ 0.999) as is the interaction between number of species and study effort ($P(\text{in best model}) = $ 0.97) compared to the benchmark random variable which has $P(\text{in best model}) = $ 0.2.
...
Models with an interaction between number of subspecies and study effort have a positive interaction term ($\beta = $ 0.14, variance = \ensuremath{3.94\times 10^{-6}}) and a negative linear term ($\beta = $ \ensuremath{-0.78}, variance = \ensuremath{2.37\times 10^{-4}}) which in total is a postive relationship for most of the range of the study effort data.
As seen in Figure~\ref{fig:ITPlots}, study effort is very likely in the best model ($P(\text{in best model}) = $ 1) body mass and range size are also probably in the best model ($P(\text{in best model}) = $ 0.8 and $P(\text{in best model}) = $ 0.66).
...
While less strongly supported than the number of subspecies, gene flow is likely in the best model ($P(\text{in best model}) = $ 0.56) compared to the benchmark random variable which has $P(\text{in best model}) = $ 0.38 as shown in Figure~\ref{fig:fstITPlots}.
On average (mean weighted by Akaiki weights) there is a negative relationship between gene flow and viral richness ($\beta = $ \ensuremath{-0.21}, variance = 0.01) despite the apparent positive relationship in Figure~\ref{fig:fstRawData}.
This supports the hypothesis that population structure promotes viral richness.
Possibly due to the smaller sample size, or a weaker relationship, this coefficient is much more varied than the number of subspecies coeficient with 23.5\% of models estimating a positive relationship.
As in the number of subspecies analysis, study effort is very likely in the best model ($P(\text{in best model}) = $ 0.94) as is body mass ($P(\text{in best model}) = $ 0.9).
In contrast to the number of subspecies analysis, range size is almost certainly not in the best model with $P(\text{in best model}) = $ 0.07 being much less than the random variable.
