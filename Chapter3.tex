...
To minimise the effects of this I only used data from studies that cover 30\% of the diameter of the species range.
...
Statistical analysis for both dependant variables were conducted using a information theory/model averaging approach \cite{burnham2002model} specifically following \cite{whittingham2005habitat}.
...
I fitted phylogenetic regressions using caper \cite{caper} to all models.
...
$\kappa$ and $othere param$ were constrained to one.
...
Following \cite{wittinghma} I included a uniformally random variable as a null variable as even unimportant variables can have Akaiki weights significantly greater than zero.
...
$\Delta\text{AICc}$ was calculated as $\text{min}(\bar{AICc}) - \bar{AICc}$, not the mean of the individual $\Delta\text{AICc}$ scores, to guarentee that the best model has $\Delta\text{AICc} = 0$.
