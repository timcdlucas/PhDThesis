...
Summing the Akaiki weights of all models that contain a given variable gives a probability, $Pr$, that the variable would be in the best model (Figure~\ref{fig:ITPlots}) if the data were recollected \cite{whittingham2006we}.
...
& \multicolumn{2}{c}{\textit{Number of Subspecies}} & \multicolumn{2}{c}{\textit{Gene flow}}\\\cmidrule{2-5}
\normalsize{Variable} & Weight & Coefficient & Weight & Coefficient\\
\toprule
Number of subspecies &&&&\\
\hspace{3mm}Total & 1 & \ensuremath{-0.75} &&\\
\hspace{3mm}Models without interaction term &&  0.19 &&\\
\hspace{3mm}Models with interaction term &&  \ensuremath{-0.78} &&\\
Number of subspecies:log(Scholar) &  0.97 &  0.14 && \\
&&&&\\
Gene flow & & &  0.61 &  0.02\\
&&&&\\
log(Scholar) &  1 &  0.35 & 
