...
These two measures of population structure were analysed separately as the number of subspecies has 192 data points while there is only $F_{ST}$ data for $\sim 30$ bat species.
...
The log number of citations on Pubmed and Google scholar were highly correlated (pgls: $t$ = 18.69, df = 190, $p$ = 0).
...
After data cleaning there was data for 192 bat species in 11 families.
...
This is corroborated by the small estimated size of $\lambda$ ($\lambda$ = 0.05, $p$ = 0.1).
...
Of the explanatory variables, the number of subspecies has no phylogenetic autocorrelation ($\lambda$ = \ensuremath{6.68\times 10^{-5}}, $p$ = 1), study effort has weak but significant autocorrelation ($\lambda$ = 0.06, $p$ = 0.04) and mass is strongly phylogenetic ($\lambda$ = 0.83, $p$ = \ensuremath{2.51\times 10^{-43}}).
...
The main model with mass, study effort and number of subspecies as predictors found study effort to be highly significant ($\beta = $ NA, $p = $ \ensuremath{1.18\times 10^{-8}}). 
The number of subspecies was marginally significant ($\beta = $ NA, $p = $ 0.03). 
The effect of nonindependance due to phylogeny was very small ($\lambda = $ 0.11, $p = $ 0.01).

The interaction term between study effort and number of subspecies, when included, was significant ($\beta = $ 0.15, $p = $ \ensuremath{2\times 10^{-3}}).
...
\begin{table}[t]
  ...
  \hline
  Number of subspecies & 0.18 (0.06 -- 0.3) & \ensuremath{4.49\times 10^{-3}}\\
  Mass & 0.33 (\ensuremath{-0.01} -- 0.68) & 0.06\\
  Study effort & 0.82 (0.54 -- 1.09) & \ensuremath{3.53\times 10^{-8}}\\
  Intercept & \ensuremath{-4.08} (\ensuremath{-6.06} -- \ensuremath{-2.11}) & \ensuremath{7.33\times 10^{-5}}\\
  \end{tabular}
