...
As these factors are completely interdependant, it is very difficult to study them empirically \emph{e.g.}, in a comparative frame work.
...
There is disagreement on whether population density (individuals per unit area) should be preferred over population size (number of individuals) \cite{begon2002clarification} and how exactly area should be incorporated \cite{begon2002clarification}. 
...
Again however, the relationships between these terms and the more global terms mentioned above are rarely examined.
...
The amount of movement between groups is at least partially dependant on the distance between groups \cite{jenkins2010meta, le2012patterns, schooley2009enhancing}.
...
Therefore there is great need to mechanistic models that try to disentangle the interplay between these many factors: density, population size, range size, population structure, group size and the number of groups.
...
Note that this is the same data as Figure~\ref{fig:plotValueChangeMeans} but on a different scale.
...
\end{align}
where $d, n$ and $m$ are density, colony size and number of colonies respectively and $c$ is a fitted intercept, $\epsilon$ is a binomially distributed error term and $b_i$ are the regression coefficients.
...
This study suggests that it is in fact population size that is important, expecially in the context of social species.
...
For example it is expected that group size increases with range size \cite{}. %TODO
